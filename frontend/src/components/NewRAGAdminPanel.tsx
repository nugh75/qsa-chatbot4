...
export default function NewRAGAdminPanel() {
  ...
  const handleUploadFiles = async () => {
    ...
    try {
      const response = await ragApiService.uploadDocuments(ragGroups.selectedGroup.id, uploadFiles);
      
      if (response.success) {
        showSnackbar(`Caricati ${uploadFiles.length} file con successo`);
        ...
        showSnackbar(response.error || 'Errore durante il caricamento', 'error');
      }
    } catch (error) {
  ...
  const handleCleanupOrphans = async () => {
    try {
      const response = await ragApiService.cleanupOrphanChunks();
      
      if (response.success) {
        showSnackbar(`Rimossi ${response.deleted_chunks || 0} chunk orfani`);
        ...
        showSnackbar(response.error || 'Errore durante la pulizia', 'error');
      }
    } catch (error) {
