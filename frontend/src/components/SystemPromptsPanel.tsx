...
import { SystemPromptEntry } from '../types/admin'

interface SystemPromptsResponse { active_id: string | null; prompts: SystemPromptEntry[] }

const SystemPromptsPanel: React.FC = () => {
  const [items, setItems] = useState<SystemPromptsResponse>({ active_id: null, prompts: [] })
  const [loading, setLoading] = useState(false)
  const [dialogOpen, setDialogOpen] = useState(false)
  const [editing, setEditing] = useState<SystemPromptEntry | null>(null)
  ...
  const openEdit = (p: SystemPromptEntry) => { setEditing(p); setName(p.name); setText(p.text); setDialogOpen(true) }
  ...
  const activate = async (id: string) => {
    await authFetch(`${BACKEND}/api/admin/system-prompts/activate?prompt_id=${encodeURIComponent(id)}`, { method: 'POST' })
    load()
  }

  const remove = async (id: string) => {
    if (!confirm('Eliminare il prompt?')) return
    await authFetch(`${BACKEND}/api/admin/system-prompts/${id}`, { method: 'DELETE' })
    load()
  }
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2 }}>
      <Stack direction="row" spacing={1} alignItems="center">
        <Typography variant="subtitle1" sx={{ flex: 1 }}>System Prompts</Typography>
